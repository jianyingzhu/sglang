...
class GemmARLayer:
    ...
    def forward(self, input_tensor: torch.Tensor, weight: torch.Tensor, bias: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # print(f"input_tensor_3d: {input_tensor_3d.shape, input_tensor_3d.stride(), input_tensor_3d.dtype}")
        # print(f"weight_3d: {weight_3d.shape, weight_3d.stride(), weight_3d.dtype}")
        ...
        if self.all_reduce == "none":
            self.compiled_gemm(a_tensor, b_tensor, self.c_tensor, current_stream)
        else:
            self.compiled_gemm(
                a_tensor, b_tensor, self.c_tensor, current_stream,
                c_mc=self.c_tensor_mc,
                barrier_flag=self.barrier_flag_memref,
                barrier_flag_mc=self.barrier_flag_mc_memref,
            )
